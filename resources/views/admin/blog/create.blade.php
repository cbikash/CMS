...
<div class="row">
    <div class="container">
        ...
        <div class="col-md-12 col-space">
            <h3>Add Blog</h3>
            <hr>
              {!! Form::open(['method'=>'post','action'=>'App\Http\Controllers\BlogController@store','files'=>true])!!}
                <div class="form-row">
                ...
                <div class="form-group">
                    <label></label>
                    <textarea name="content" rows="5" id="ckeditor" class="form-control">{{old('content')}}</textarea>
                    @error('content')
                    <small class="form-text text-danger" style="font-size: 17px">{{$message}}</small>
                    @enderror
                </div>

                {!! Form::submit('Add Content',['class'=>'btn btn-primary shadow']) !!}
                {!! Form::close() !!}
        </div>
    </div>
</div>
